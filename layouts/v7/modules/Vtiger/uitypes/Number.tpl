...
{if $FIELD_MODEL->getFieldDataType() eq 'integer'}
    data-rule-maxlength="{$FIELD_MODEL->maximumlength}"
{else if $FIELD_MODEL->getFieldDataType() eq 'double'}
    {* 少数の場合は、整数部分の最大値を上限とする *}
    data-rule-max="{str_repeat('9', $FIELD_MODEL->maximumlength)}"
{/if}
