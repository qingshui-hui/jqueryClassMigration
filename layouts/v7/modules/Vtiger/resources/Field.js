...
jQuery.Class("Vtiger_Field_Js",{
	...
	/**
	 * Function to get maximum length of the object
	 * @return maximum length attribute of the object
	 */
	 ...
	 */
	getUi : function() {
		var html = '<input class="inputElement" type="text" name="'+ this.getName() +'" data-label="'+this.get('label')+'"'
			+' data-rule-'+this.getType()+'="true"'
			+' />';
...
Vtiger_Field_Js('Vtiger_Currency_Field_Js',{},{
	...
	getUi : function() {
		...
		var html = '<div class="referencefield-wrapper"><div class="input-group">'+
						...
						'<input class="inputElement" type="text" name="'+ this.getName()+'"'
							+' data-rule-currency="true" value="'+ this.getValue()+'"'
							+' data-rule-currencymax="'+'9'.repeat(this.getMaximumLength())+'"'
							+' />'+
					'</div></div>';
		var element = jQuery(html);
		return this.addValidationToElement(element);
...
Vtiger_Field_Js('Vtiger_Percentage_Field_Js',{},{
	...
	getUi : function() {
		var html = '<div class="input-group percentage-input-group">'+
						'<input type="text" class="form-control inputElement percentage-input-element" name="'+this.getName() +'"'
							+' value="'+ this.getValue() + '" step="any" data-rule-'+this.getType()+'="true"'
							+' data-rule-max="100"'
							+' />'
...
Vtiger_Field_Js('Vtiger_String_Field_Js',{},{
	getUi : function() {
		var html = '<input class="inputElement" type="text" name="'+ this.getName() +'" data-label="'+this.get('label')+'"'
			+' data-rule-'+this.getType()+'="true"'
			+' data-rule-maxlength="'+this.getMaximumLength()+'"'
			+' />';
		html = jQuery(html).val(app.htmlDecode(this.getValue()));
		return this.addValidationToElement(html);
	}
...
Vtiger_Field_Js('Vtiger_Double_Field_Js',{},{
	getUi : function() {
		var html = '<input class="inputElement" type="text" name="'+ this.getName() +'" data-label="'+this.get('label')+'"'
			+' data-rule-'+this.getType()+'="true"'
			+' data-rule-max="'+'9'.repeat(this.getMaximumLength())+'"'
			+' />';
		html = jQuery(html).val(app.htmlDecode(this.getValue()));
		return this.addValidationToElement(html);
	}
